...
void thomasTriDiagonal(int j, double a[],double b[],double c[], double d[], struct NavierStoke *cavity){
	...
	for (int i = imax-2;i>=3;i--){
		...
		// u[i] = (cprime[j]-(a[j]*u[i+1]))/(dprime[j]);
	}  
}
  ...
  // // diagonalX[1] = 1.;
	...
	for(int lsoriter=0; lsoriter <= cavity->lsormax; lsoriter++) {
		...
		for (int  j = 2; j<=jmax-1;j++){
			...
			for (int i = 2; i<=imax-1;i++){			
				...
				//SET UP THOMAS VECTORS
					...
					third[i] = (cavity->dilation_N[j][i])/dt;
      ...
      //dPsi/dX = 0 on left and right wall 
			...
			//CALL THOMAS
			thomasTriDiagonal(j,ax,bx,cx,diagonalX,csavity);
...
// CALCULATE U
	...
	for (int j = 2; j<=jmax-1;j++){
		for (int i = 2; i<=imax-1;i++){
			...
			cavity->v_updated[j][i] = cavity->v[j][i] + dt*(((1./reynolds)*((cavity->v[j][i+1]-2.*cavity->v[j][i]+cavity->v[j][i-1])/(pow(dx,2.))+(cavity->v[j+1][i]-2.*cavity->v[j][i]+cavity->v[j-1][i])/(pow(dy,2.))))-(((cavity->u[j][i+1]*cavity->v[j][i+1])-(cavity->u[j][i-1]*cavity->v[j][i-1]))/(2.*dx))-(0.5*(1+epsilonY)*(pow(cavity->v[j][i],2.)-pow(cavity->v[j-1][i],2.))/(dy))-(0.5*(1-epsilonY)*(pow(cavity->v[j+1][i],2.)-pow(cavity->v[j][i],2.))/(dy))-(0.5*(1+epsilonY)*(cavity->p_updated[j][i]-cavity->p_updated[j-1][i])/(dy))-(0.5*(1-epsilonY)*(cavity->p_updated[j+1][i]-cavity->p_updated[j][i])/(dy)));
		}
	}
