...
//Thomas Algorithm
double  * thomasTriDiagonal(int j, double a[],double b[],double c[], double d[], struct NavierStoke *cavity){
		...
		for (int  j = 2; j<=jmax-1;j++){
			...
			for (int i = 2; i<=imax-1;i++){			
				...
				//SET UP THOMAS VECTORS
					...
					second[i] = (cavity->w_optimal*BetaSquare)*(cavity->p[j+1][i]+cavity->p[j-1][i]);
