...
void DufortFrankelEXPLICIT(double t0, double tboundary , int imax, int nmax, double dt, double dx, double x[], FILE* outfile2, double out_un[]);
...
int main(){
	...
	for (int k = 0; k < 5; k++){
		...
		nmax_vector[k] = (int)lround(t_target/delta_t);

		FTCS_explicit(t0,tboundary,imax,nmax_vector[k],delta_t,diffusion,x_vector,outfile1,Tfinal);

		fprintf(outfile1, "# t = %.2f hr\n", t_target);
		fprintf(outfile1, "# x\tT\n");// make this a comment too

	for (int i = 1; i <= imax; i++) {
    	fprintf(outfile1, "%.6f\t%.6f\n", x_vector[i], Tfinal[i]);
	}

	fprintf(outfile1, "\n\n");// two blank lines, required for index

	}

	fclose(outfile1);
	int ret = system("gnuplot ftcs_explicit.gp");
	if (ret != 0) {
