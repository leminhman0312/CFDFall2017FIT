...
// Calculations
double thomasTriDiagonal(int imax, double a[], double b[], double c[],
                         double d[], double u[], double x[]);
double* FTCS_return_implicit(int nmax, double dx, double tboundary, double t0,
                             int imax, double x[]);

// Finite Difference Methods
double FTCS_explicit(double t0, double tboundary, int imax, int nmax, double dt,
                     double dx, double x[], FILE* outfile1);
double DufortFrankelEXPLICIT(double t0, double tboundary, int imax, int nmax,
                             double dt, double dx, double x[], FILE* outfile2);
double FTCS_implicit(int nmax, double dx, double tboundary, double t0, int imax,
                     double x[], FILE* outfile3);
double CrankNicolson_implicit(int nmax, double dx, double tboundary, double t0,
                              int imax, double x[], FILE* outfile4);
...
int main() {
  ...
  // FILES to output.  Change dt value above then comment out the appropriate
  // sections
  ...
  // int nmax_vector[5] = {0,10,20,30,40}; //This is to store the step size at
  // 0.1, 0.2,0.3 and 0.4 hr

  // for (int i = 1; i<=4;i++){
  // ftcs_explicit_value[i][imax] =
  // FTCS_explicit(t0,tboundary,imax,nmax_vector[i],dt,dx,x_vector,outfile1);
  // DufortFrankel_value[i][imax] =
  // DufortFrankelEXPLICIT(t0,tboundary,imax,nmax_vector[i],dt,dx,x_vector,outfile2);
  // ftcs_implicit_value[i][imax] =
  // FTCS_implicit(nmax_vector[i],dx,tboundary,t0,imax,x_vector,outfile3);
  // CrankNicolson_value[i][imax] =
  ...
  // int nmax_vector[5] = {0,2,4,6,8}; //This is to store the step size at 0.1,
  // 0.2,0.3 and 0.4 hr
...
double FTCS_explicit(double t0, double tboundary, int imax, int nmax, double dt,
  ...
  for (int n = 1; n <= nmax; n++) {
    ...
    for (int i = 2; i <= imax - 1; i++) {
      ...
      u[i] = u[i] + dx * (u[i + 1] - (2 * u[i]) + u[i - 1]);
    }
  }
  // Printing to file
  for (int k = 1; k <= imax; k++) {
    fprintf(outfile1, "%f\t", u[k]);
    printf("%f\n", u[k]);
  }
...
double FTCS_implicit(int nmax, double dx, double tboundary, double t0, int imax,
  ...
  for (int t = 1; t <= nmax; t++) {
    for (int i = 1; i <= imax; i++) {
      thomasTriDiagonal(imax, a_FTCS_implicit, b_FTCS_implicit, c_FTCS_implicit,
                        d_FTCS_implicit, u_FTCS_implicit, x);
...
double CrankNicolson_implicit(int nmax, double dx, double tboundary, double t0,
  ...
  for (int t = 1; t <= nmax; t++) {
    for (int i = 1; i <= imax; i++) {
      thomasTriDiagonal(imax, a_NC_implicit, b_NC_implicit, c_NC_implicit,
                        d_NC_implicit, u_NC_implicit, x);
...
double* FTCS_return_implicit(int nmax, double dx, double tboundary, double t0,
  ...
  for (int t = 1; t <= nmax; t++) {
    for (int i = 1; i <= imax; i++) {
      thomasTriDiagonal(imax, a_FTCS_implicit_return, b_FTCS_implicit_return,
                        c_FTCS_implicit_return, d_FTCS_implicit_return,
                        u_FTCS_implicit_return, x);
      c_FTCS_implicit_return[i] = -1 * u_FTCS_implicit_return[i];
    }
  }

  return u_FTCS_implicit_return;  // return the pointer array to feed into
                                  // DufortFrankel
}
...
double DufortFrankelEXPLICIT(double t0, double tboundary, int imax, int nmax,
  ...
  for (int n = 1; n <= nmax; n++) {
    ...
    // FDE using the intial FTCS implict as starters
      ...
      for (int j = 2; j <= imax - 1; j++) {
        u_previous[j] = u[j];
        u[j] = (((1 - 2. * dx) / (1 + 2. * dx)) * u_previous[j]) +
               ((2. * dx) / (1 + 2. * dx)) * (u[j + 1] + u[j - 1]);
      }
    }
  }
